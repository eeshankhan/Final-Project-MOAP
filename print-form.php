...
<style type="text/css">
	...
	*/}
	...
	#stu-info-table, #parents-info-table, #education-info-table,#contact-info-table,#biographic-info-table, #preferences-info-table{
		height: 25px;
		border: 1px solid grey;
	...
	#parents-info-table-fill, #stu-info-table-fill, #education-info-table-fill, #contact-info-table-fill, #biographic-info-table-fill, #preferences-info-table-fill{
		width: 340px;
		border: 1px solid grey;
		padding-left: 5px;
	}
	...
	*/}
	...
	#biographic-table{page-break-after: always;}
	}
...
<body>
  ...
  $sql1 = "SELECT * FROM tbl_personal WHERE serial_no='$search' ";
  ...
  if (mysqli_num_rows($result) > 0) {
   ...
   $sql1 = "SELECT * FROM tbl_education WHERE serial_no='$search' ";
   $result = $conn->query($sql1);
   if (mysqli_num_rows($result) > 0) {
         $row = mysqli_fetch_assoc($result);
            $serial_no=  $row['serial_no'];
    ...
    $sql1 = "SELECT * FROM tbl_contact WHERE serial_no= '$search'";
    $result = $conn->query($sql1);
    if (mysqli_num_rows($result) > 0) {
          $row = mysqli_fetch_assoc($result);
             $serial_no=  $row['serial_no'];
     ...
     $sql1 = "SELECT * FROM tbl_biographic WHERE serial_no='$search' ";
     $result = $conn->query($sql1);
     if (mysqli_num_rows($result) > 0) {
           $row = mysqli_fetch_assoc($result);
              $serial_no=  $row['serial_no'];
      ...
      $sql1 = "SELECT * FROM tbl_signature WHERE serial_no='$search' ";
      $result = $conn->query($sql1);
      if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
               $serial_no=  $row['serial_no'];
      ...
      $sql1 = "SELECT * FROM tbl_preferences WHERE serial_no='$search' ";
      $result = $conn->query($sql1);
      if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
               $serial_no=  $row['serial_no'];
               $faculty_1= $row['faculty_1'];
               $branch_1= $row['branch_1'] ;
               $faculty_2= $row['faculty_2'];
               $branch_2= $row['branch_2'] ;
               $faculty_3= $row['faculty_3'];
               $branch_3= $row['branch_3'] ;
               $faculty_4= $row['faculty_4'];
               $branch_4= $row['branch_4'] ;
               $faculty_5= $row['faculty_5'];
               $branch_5= $row['branch_5'] ;
               $count++;
               //echo "done";
      }
      else{
...
<div  style=" border: 1px solid black; margin: 10px 10px;">
	...
	<table >
		<tr>
			...
			<td style="vertical-align: top;">
				<div class="row">
					<div class="col-md-12">
						<center><img src="img/logo.png" style="width: 70%; height: 80%;"></center>
					</div>
				</div>
			</td>
	...
	<table id="serial-table" style="margin-top: 20px; margin-left: 0px; font-weight: bold; border-top: 3px solid black; border-bottom: 3px solid black; font-size: 20px">
		<tr height="45">
			...
			<td id="serial-info-table" style="width: 170px;">&nbsp; Name:</td>
			<td id="serial-info-table-fill" style="width: 250px;"><?php echo $student_name ?></td>
...
<table id="preferences-table" style="margin: 20px;">
	<tr>
		<td id="preferences-info-table">1.</td>
		<td id="preferences-info-table-fill"><?php echo $faculty_1.'-'.$branch_1 ?></td>
	</tr>

	<tr>
		<td id="preferences-info-table">2.</td>
		<td id="preferences-info-table-fill"><?php echo $faculty_2.'-'.$branch_2 ?></td>
	</tr>

	<tr>
		<td id="preferences-info-table">3.</td>
		<td id="preferences-info-table-fill"><?php echo $faculty_3.'-'.$branch_3 ?></td>
	</tr>

	<tr>
		<td id="preferences-info-table">4.</td>
		<td id="preferences-info-table-fill"><?php echo $faculty_4.'-'.$branch_4 ?></td>
	</tr>

	<tr>
		<td id="preferences-info-table">5.</td>
		<td id="preferences-info-table-fill"><?php echo $faculty_5.'-'.$branch_5 ?></td>
	</tr>
</table>



<button onclick='p1()' class='print' id='b1'> Print  <i class='fas fa-print' style="font-size:20px;"></i> </button>
</td></tr>
</table>
</div>
</body>
